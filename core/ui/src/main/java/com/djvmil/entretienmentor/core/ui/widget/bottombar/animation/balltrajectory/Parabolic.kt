package com.djvmil.entretienmentor.core.ui.widget.bottombar.animation.balltrajectory
...
class Parabolic(
  ...
  override fun animateAsState(targetOffset: Offset): State<BallAnimInfo> {
    ...
    return produceState(
      ...
      if (pos[0] == Float.MAX_VALUE) {
        ...
        this.value = this.value.copy(offset = calculateNewOffset(pos, ballSizePx, verticalOffset))
      }
    }
  }
  ...
  private fun Path.createParabolaTrajectory(from: Offset, to: Offset, height: Float) {
    ...
    quadTo((from.x + to.x) / 2f, from.y - height, to.x, to.y)
  }

  private fun calculateNewOffset(pos: FloatArray, ballSizePx: Float, verticalOffset: Float) =
      Offset(x = pos[0] - (ballSizePx / 2), y = pos[1] - verticalOffset)
