package com.djvmil.entretienmentor.core.ui.widget.bottombar.layout
...
@Composable
fun animatedNavBarMeasurePolicy(onBallPositionsCalculated: (ArrayList<Float>) -> Unit) = remember {
...
internal fun barMeasurePolicy(onBallPositionsCalculated: (ArrayList<Float>) -> Unit) =
    MeasurePolicy { measurables, constraints ->
      check(measurables.isNotEmpty()) { "There must be at least one element" }
...
fun calculateGap(placeables: List<Placeable>, width: Int): Int {
  var allWidth = 0
  placeables.forEach { placeable -> allWidth += placeable.width }
