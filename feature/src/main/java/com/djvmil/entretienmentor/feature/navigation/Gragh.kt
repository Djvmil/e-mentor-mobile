...
fun NavGraph(
    ...
    val startDestination: String = when(appSettings.stepsStarting){
        StepsStarting.ON_BOARDING -> { Destinations.ON_BOARDING_ROUTE }
        StepsStarting.ON_HOME_USER -> { Destinations.HOME_ROUTE }
        StepsStarting.ON_GUEST_PAGE -> { Destinations.HOME_GUEST_ROUTE }
        StepsStarting.ON_AUTH_PAGE -> { Destinations.AUTH_ROUTE }
    }

    Log.e("NavGraph", "startDestination: $startDestination ", )
    ...
    NavHost(
        ...
        home(
            navActions = navActions,
            isGuest = startDestination == Destinations.HOME_GUEST_ROUTE
        )
        onBoarding(
            navActions = navActions
        )
